sub dijkstra {
    my($graph, $src, $init_distance) = @_;

    $init_distance = 0 unless defined $init_distance;
    
    my $q = new List::PriorityQueue;
    my %dist;
    my %prev;

    for my $v (sort keys %{ $graph }) {
        $dist{$v} = 1_000_000_000;
        $prev{$v} = undef;
        $q->insert($v, 1_000_000_000);
    }

    $dist{$src} = $init_distance;
    $q->update($src, $init_distance);

    while (1) {
        my $u = $q->pop();
        last unless defined $u;
        
        for my $e (sort keys %{ $graph->{$u} }) {
            # print "examining $u -> $e\n";
            my $alt = $dist{$u} + $graph->{$u}{$e};
            if (!defined $dist{$e} || $alt < $dist{$e}) {
                $dist{$e} = $alt;
                $prev{$e} = $u;
                $q->update($e, $alt);
                # print "\tupdating $e\n";
            } else {
                # print "\tignoring $e because new distance $alt is the same as old distance $dist{$e}\n";
            }
        }
    }
